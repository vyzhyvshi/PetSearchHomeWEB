...
namespace PetSearchHome_WEB.Application.Moderation
{
    ...
    public class BlockUserUseCase : IUseCase<BlockUserRequest, Result<bool>>
    {
        ...
        public async Task<Result<bool>> ExecuteAsync(BlockUserRequest request, AuthContext authContext, CancellationToken cancellationToken = default)
        {
            if (!AdminPolicy.IsAdmin(authContext.Role))
            {
                return Result.Failure<bool>("Немає прав доступу.");
            }
